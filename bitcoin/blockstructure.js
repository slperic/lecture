...
const 헥스를바이너리로 = require("hex-to-binary");
//16진수를 2진수로 바꾼다
...
const 제네시스블록 = new 블록(
  ...
  //해시
  "",
  //이전해시 - 제네시스 블록이기 때문에 없음!
  1569523151,
  //시간
  "",
  //데이터 = 빈자리
  0,
  //난이도 = 0 - 제네시스 블록이기 때문에 없음!
  0
  //논스 또한 제네시스 블록이기 때문에 없음
);

//블록체인 선언
let 블록체인 = [제네시스블록];
//가장 최신 블록을 가져오기 위한 인덱스 값-1
...
const 난이도더하기 = (어떤블록체인) =>
  어떤블록체인
    .map((블록) => 블록.난이도) // [1, 2, 2, 4, 5 ]
    .map((난이도) => Math.pow(2, 난이도)) // [1, 4, 4, 16, 25 ]
    .reduce((a, b) => a + b);
const 체인교체 = (후보체인) => {
  //TODO : 거래 관련 코드
  if (
